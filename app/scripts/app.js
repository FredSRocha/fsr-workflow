...
(function() {
  'use strict';
  angular.module('core', ['ng', 'ngCookies', 'ngResource', 'ngSanitize', 'ui.router', 'pascalprecht.translate']);
  angular.module('ft.example', []);
  angular.module('app', ['core', 'ft.example']);
  angular.element(document).ready(function() {
    var $http, initInjector;
    initInjector = angular.injector(['ng']);
    $http = initInjector.get('$http');
    $http.get('json/constants/app.json').then(function(response) {
...
(function() {
  var feature;
  feature = function(randomUserApi, $translate, $log, constAPP) {
    var vm;
    vm = this;
    ...
    vm.translateLanguage = function(key) {
      if (key !== null) {
        $translate.use(key.lang);
      }
    };
    vm.getRandomUserApi = function() {
      return randomUserApi.query({
        results: '10'
      }, (function(data) {
  ...
  feature.$inject = ['randomUserApi', '$translate', '$log', 'constAPP'];
})();
...
(function() {
  ...
  Config = function($stateProvider, $urlRouterProvider, $translateProvider, $locationProvider) {
    ...
    $translateProvider.useStaticFilesLoader({
      ...
      suffix: '.json'
    }).useLocalStorage().preferredLanguage('pt-BR').useSanitizeValueStrategy(null);
  };
  'use srtict';
  angular.module('ft.example').config(Config);
  Config.$inject = ['$stateProvider', '$urlRouterProvider', '$translateProvider', '$locationProvider'];
})();

(function() {
  var randomUserApi;
  randomUserApi = function($resource) {
    return $resource('https://randomuser.me/api/?results=:results', {}, {
      query: {
        method: 'get',
        results: '@results',
        isArray: false
